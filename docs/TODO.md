...
| ~~**F3 v1**~~ | ~~Export PDF des trades — Pro only~~ ✅ 2026-05-14 (v0.9.132) | Bouton "Exporter PDF" dans Settings → modal date range + compte → PDF avec page de garde (stats globales) + pages trades (6/page : instrument, direction, niveaux, P&L, R:R, setup, notes). jsPDF bundlé local (CSP-safe), 100% client-side. Phase 4 (screenshots embarqués) à venir si retour user positif. | ✅ v1 |
