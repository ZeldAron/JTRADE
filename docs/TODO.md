...
| **B5** | Contact form dans l'app cassé après v0.9.161 fail-closed hCaptcha (signalé user 2026-05-16) | Reproduction : connecté dans l'app → cliquer bulle Contact → remplir form → Envoyer → échec. Cause probable : `_verifyHcaptcha` passé en fail-closed (H-002 audit) renvoie maintenant `false` quand `HCAPTCHA_SECRET` n'est pas configuré (avant : `true` mode dégradé). À vérifier : (1) `HCAPTCHA_SECRET` est-il vraiment configuré en prod (cf. `firebase functions:secrets:access HCAPTCHA_SECRET`) ? (2) Si oui, le token hCaptcha côté client est-il bien envoyé ? Fichiers : `functions/index.js:_verifyHcaptcha`, `src/js/contact.js`, `src/app.html#contactForm`. Effort : 30 min (debug + fix). | ⏳ À fixer |
| **B6** | Plus de formulaire de contact sur la landing page index.html (signalé user 2026-05-16) | Actuellement la landing page n'a que des liens `mailto:zeldtradepro@gmail.com` (lignes 1326, 1368 de `src/index.html`). Avant il y avait un vrai form. À recréer : section "Contact" avec form (nom + email + message) + hCaptcha + envoi via CF `sendContactMessage`. Vérifier que la CF accepte un appel sans auth (visiteurs anonymes de la landing). Effort : 1-2h (UI + binding + test). | ⏳ À fixer |
...
| **F3 v1.5** | **Export PDF des trades — Pro only (à re-tester)** ⚠️ 2026-05-14/15 (v0.9.132 → v0.9.136) | Bouton "Exporter PDF" dans Settings → modal date range + compte → PDF avec page de garde (stats globales) + pages trades (6/page). **Code déployé v0.9.136 mais pas re-testé live par l'user.** Bugs identifiés et corrigés (à valider) : (1) handler bouton via event delegation v0.9.135 ; (2) champs trades corrigés v0.9.136 : `t.timestamp` → `t.date` (ISO string), `t.netPnl` → `Calc.trade(t).netPnl`, `t.rr` → `Calc.trade(t).rr`, `t.account` → `t.apex`, `→` → "du... au..." (jsPDF helvetica) ; (3) autorise 0 trade (page de garde seule) v0.9.134 ; (4) check Pro à chaque render v0.9.133. **À faire** : Cmd+Shift+R + tester click bouton → vérifier que modal s'ouvre + que PDF contient les trades. **Phase 4 (Phase future) : Screenshots embarqués depuis Cloud Storage.** | À re-tester |
