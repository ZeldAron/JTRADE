...
| **F4 v2** | **Landing page v2 — finitions** | Screenshots réels de l'app (Journal, Dashboard, Wizard) avec lazy-loading. Témoignages bêta-testeurs (quand récoltés). Routing automatique : non-loggé sur `/` → redirect `landing.html`, loggé → reste sur app. Logo SVG custom (au lieu du Z inline). Animations subtle (fade-in scroll, hover features cards). Brancher landing.html à la racine GitHub Pages si désiré. | ~4-6h |
