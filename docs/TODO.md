...
| **F3** | **Export PDF des trades — Pro only** | Bouton "Exporter PDF" dans Settings (ou page dédiée) → permet de sélectionner une **période** (date début / date fin), génère un **PDF complet** incluant pour chaque trade : instrument, direction, entry/SL/TP, P&L, R:R, setup, notes, **screenshot** (si présent), partial close info. Une page de garde avec stats globales de la période (total P&L, winrate, R:R moyen). **Restriction : Pro uniquement** (`Store.isPro()`). Lib client-side : `jsPDF` + `html2canvas` (ou `pdfmake`) — pas de Cloud Function nécessaire. Pertinent pour comptabilité prop firm, partage avec coach, archivage perso. | À définir (~6-8h) |
| **F4** | **Landing page propre** | Page d'accueil dédiée pour les visiteurs non-loggés (actuellement on tombe directement sur l'écran de login). **Sections suggérées** : hero (titre + tagline + CTA "Essayer"), 3-4 features clés (journal illimité, IA chart, prop firms, calculs précis), screenshots app, témoignages bêta-testeurs (quand dispo), pricing (modèle stealth → "Sur invitation" ou "Contactez-nous"), FAQ, footer (mentions légales, contact). **Design** : sombre cohérent avec l'app, responsive mobile-first, performance (lazy load images, SVG inline). **Routage** : `/landing.html` ou root path qui détecte si user loggé → redirect app, sinon affiche landing. | À définir (~10-15h) |
